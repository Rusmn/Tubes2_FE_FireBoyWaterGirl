...
export const searchRecipe = async (params) => {
	try {
		...
		if (!response.ok) {
			...
			throw new Error(errorData.message || 'Terjadi kesalahan saat mencari recipe');
		}
...
export const getAllElements = async () => {
	try {
		...
		if (!response.ok) {
			const errorData = await response.json();
			throw new Error(errorData.message || 'Terjadi kesalahan saat mengambil daftar elemen');
		}
